...
| `tools/archive-reporter.sh` (repo thinning) | skill | yes | yes | yes | active | PM | Read-only report of closed+aged coordination units (mail/plans/chat); prints `git rm` + `ARCHIVED.md` row for a human/orchestrator to run at milestone start. Never mutates, no `run` subcommand, no `MINION_*` gate. PM runs it at each milestone/run start and acts on the candidate list; the `unmarked-but-aged` line surfaces closure-marking drift. See `docs/archive-reporter-model.md` |
...
| `/onboard` (session onboarding) | skill | yes | yes | yes | active | all roles | Claude invokes via the dedicated `/onboard` launcher (`.claude/commands/onboard.md`); Codex/Copilot orchestrators run the identical protocol from the tool-neutral Onboarding Mode in `docs/minion-prompt-modes.md` — no dedicated command file needed there. Deterministic session-start onboarding: executes the CLAUDE.md read-chain, reads docs/MECHANICS.md (flags staleness), checks minions/handoffs/ (absorb, hold the delete), surfaces MINION_* gates, emits a ready-state report. Read-only. See the design spec. |
