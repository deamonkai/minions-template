...
`upgrade-classify.sh`, `xtool-call.sh`, `layer-adopted.sh`) and the test suite
under `tools/tests/*.test.sh` (`governance-consistency.test.sh`,
...
- Locale-collation portability: any test that compares `sort` output
  against a hard-coded expected ordering must pin `LC_ALL=C` on that
  `sort` — macOS's default UTF-8 locale collates case-insensitively and
  false-fails C-order expectations on a pristine clone (downstream field
  report, v1.37.0). Same-run sorted-vs-sorted comparisons and
  order-independent iteration need no pin.
