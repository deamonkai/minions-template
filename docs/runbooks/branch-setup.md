...
curl -s -X PATCH \
  ...
  "https://git.example.com/api/v1/repos/Your-Org/minions-template/branches/main/protection" \
  -d '{
    "enable_push": false,
...
for BRANCH in dev staging; do
  curl -s -X PATCH \
    -H "Authorization: token <your-personal-access-token>" \
    -H "Content-Type: application/json" \
    "https://git.example.com/api/v1/repos/Your-Org/minions-template/branches/${BRANCH}/protection" \
    -d '{
      "enable_push": true,
...
curl -s -X POST \
  -H "Authorization: token <your-personal-access-token>" \
  -H "Content-Type: application/json" \
  "https://git.example.com/api/v1/repos/Your-Org/minions-template/pulls" \
  -d '{
